...
| `OIDC_ENFORCE_IDP_TOKEN_EXPIRY` | Boolean | `true` | When `false`, the IdP-issued token expiry is ignored: neither it nor the refresh token is stored, the refresh flow is never entered, and the browser session lives for `SESSION_COOKIE_MAX_AGE_SECONDS` (revocable server-side). Set this when the IdP rotates refresh tokens with reuse detection — a browser issuing requests in parallel refreshes several times at once with the same token, the IdP honours the first and treats the rest as replay, and revoking the chain ends the session the refresh was meant to extend. The cost is that IdP-side changes no longer take effect within the token's lifetime; they wait for session expiry or an explicit revoke here, so pair it with a shorter `SESSION_COOKIE_MAX_AGE_SECONDS` if that window matters |
